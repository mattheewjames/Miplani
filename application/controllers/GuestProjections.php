...
class GuestProjections extends MY_Web_Controller 
{
 	...
 	public function index()
	{
		...
		if(!empty($_GET['key']))
		{
			...
			$data['row'] = $this->users->get_invite_projection_row($key_url);
			if(!empty($data['row']))
			{
				if($data['row']->projection_type=='sub')
				{
					$data['projection_row'] = $this->projection->get_guest_projection_details($data['row']->projection_id,$key_url);
					if(!empty($data['projection_row']))
					{
		...
		load_web_main_template('guest_sub_projections_v', $header_data,$data);
	}
	public function ProjectionGuestStep()
	{
		if($this->input->server('REQUEST_METHOD')=='POST')
		{
			...
			else
			{
				...
				{
					$projection_row = $this->projection->get_guest_projection_details($projection_id,$key_url);
					if(!empty($projection_row))
					{
						$user_row = $this->users->get_active_user_row($projection_row->user_id);
						if($this->input->post('form_step')==1)
						{
							if($this->input->post('action_type')=='save' || $this->input->post('action_type')=='next')
							{
								...
								{
									$peojection_id = $this->projection->save_guest_projection($key_url);
									$html = $this->load_projection_step_one_html($peojection_id,$action_type,$key_url);
								}
							}
							else
							{
						...
						elseif($this->input->post('form_step')==2)
						{
							if($this->input->post('action_type')=='save' || $this->input->post('action_type')=='next')
							{
								...
								{
									$peojection_id = $this->projection->save_guest_projection($key_url);
									$html = $this->load_projection_step_two_html($peojection_id,$action_type,$key_url);
								}
							}
							else
							{
						...
						elseif($this->input->post('form_step')==3)
						{
							if($this->input->post('action_type')=='save' || $this->input->post('action_type')=='next')
							{
								...
								{
									$peojection_id = $this->projection->save_guest_projection($key_url);
									$html = $this->load_projection_step_three_html($peojection_id,$action_type,$key_url);
								}
							}
							else
							{
						...
						elseif($this->input->post('form_step')==4)
						{
							if($this->input->post('action_type')=='save' || $this->input->post('action_type')=='next')
							{
								...
								{
									$peojection_id = $this->projection->save_guest_projection($key_url);
									$html = $this->load_projection_step_four_html($peojection_id,$action_type,$key_url);
								}
							}
							else
							{
						...
						elseif($this->input->post('form_step')==5)
						{
							if($this->input->post('action_type')=='save' || $this->input->post('action_type')=='next')
							{
								...
								{
									$peojection_id = $this->projection->save_guest_projection($key_url);
									$html = $this->load_projection_step_five_html($peojection_id,$action_type,$key_url);
								}
							}
							else
							{
						...
						elseif($this->input->post('form_step')==6)
						{
							...
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
								$html = $this->load_projection_step_six_html($peojection_id,$action_type,$key_url);
							}
							else
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
						...
						elseif($this->input->post('form_step')==7)
						{
							...
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
								$html = $this->load_projection_step_seven_html($peojection_id,$action_type,$key_url);
							}
							else
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
						...
						elseif($this->input->post('form_step')==8)
						{
							...
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
								$html = $this->load_projection_step_eight_html($peojection_id,$action_type,$key_url);
							}
							else
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
						...
						elseif($this->input->post('form_step')==9)
						{
							...
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
								$html = $this->load_projection_step_nine_html($peojection_id,$action_type,$key_url);
							}
							else
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
						...
						elseif($this->input->post('form_step')==10)
						{
							...
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
								$html = $this->load_projection_step_ten_html($peojection_id,$action_type,$key_url);
							}
							else
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
						...
						elseif($this->input->post('form_step')==11)
						{
							...
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
								$html = $this->load_projection_step_eleven_html($peojection_id,$action_type,$key_url);
							}
							else
							{
								$peojection_id = $this->projection->save_guest_projection($key_url);
						...
						elseif($this->input->post('form_step')==12)
						{
							...
							if($this->input->post('action_type')=='save' || $this->input->post('action_type')=='submit')
							{
								...
								else
								{
									...
									{
										$html = $this->load_projection_step_twelve_html($peojection_id,$action_type);	
									}
								}
							}
							else
						...
						if(!empty($html))
						{
							if($html=='completed')
							{
								$response_id = base64_encode($peojection_id);
	...
	public function GuestModifyProjection()
	{
		if($this->input->server('REQUEST_METHOD')=='POST')
		{
			$html ='';
			...
			if($form_step>0 && $form_step<=12)
			{
				$row = $this->projection->get_guest_projection_details($projection_id,$key_url);
				if(!empty($row))
				{
					if($this->input->post('form_step')==1)
					{
						 $html = $this->load_projection_step_one_html($projection_id,$action_type,$key_url);
					}
					elseif($this->input->post('form_step')==2)
					{
						$html = $this->load_projection_step_two_html($projection_id,$action_type,$key_url);	
					}
					elseif($this->input->post('form_step')==3)
					{
						$html = $this->load_projection_step_three_html($projection_id,$action_type,$key_url);
					}
					elseif($this->input->post('form_step')==4)
					{
						$html = $this->load_projection_step_four_html($projection_id,$action_type,$key_url);	
					}
					elseif($this->input->post('form_step')==5)
					{
						$html = $this->load_projection_step_five_html($projection_id,$action_type,$key_url);	
					}
					elseif($this->input->post('form_step')==6)
					{
						$html = $this->load_projection_step_six_html($projection_id,$action_type,$key_url);	
					}
					elseif($this->input->post('form_step')==7)
					{
						$html = $this->load_projection_step_seven_html($projection_id,$action_type,$key_url);	
					}
					elseif($this->input->post('form_step')==8)
					{
						$html = $this->load_projection_step_eight_html($projection_id,$action_type,$key_url);
					}
					elseif($this->input->post('form_step')==9)
					{
						$html = $this->load_projection_step_nine_html($projection_id,$action_type,$key_url);	
					}
					elseif($this->input->post('form_step')==10)
					{
						$html = $this->load_projection_step_ten_html($projection_id,$action_type,$key_url);	
					}
					elseif($this->input->post('form_step')==11)
					{
						$html = $this->load_projection_step_eleven_html($projection_id,$action_type,$key_url);
					}
					elseif($this->input->post('form_step')==12)
					{
						$html = $this->load_projection_step_twelve_html($projection_id,$action_type,$key_url);
					}
	...
	private function load_projection_step_one_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Users_m","users");
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 2;
		$projection_permission = 'view';
		if($projection_id>0)
		{
			$projection_row = $this->projection->get_guest_projection_details($projection_id,$key_url);
 			if(!empty($projection_row))
					...
					$html .= '<div class="col-lg-12 col-md-12">
							<div class="contact_message form form-bg">
								...
								<div class="row">
									<div class="offset-md-3 col-md-6">
										<p>
											...
											{
												$html .= '<input name="projection_name" id="projection_name" value="'.$projection_row->projection_name.'" placeholder="'.$this->lang->line('lang_enter_projection_name_label').'" type="text">';
											}
											else
											{
												$html .= '<input name="projection_name" id="projection_name" value="'.$projection_row->projection_name.'" placeholder="'.$this->lang->line('lang_enter_projection_name_label').'" type="text" readonly>';
											}
										$html .= '</p>
									</div>
								</div>
								...
								<div class="col-md-12 text-right p-0">';
									if(in_array($projection_guest_permission,array('update','all')))
									{
										$html .= '<button type="button" class="save-btn" onclick="load_guest_projection_wizard(\'1\',\''.$projection_id.'\',\'save\',\''.$key_url.'\')"> '.$this->lang->line('lang_save_label').'</button>';
									}
									$html .= '<button type="button" onclick="load_guest_projection_wizard(\'1\',\''.$projection_id.'\',\'next\',\''.$key_url.'\')"> '.$this->lang->line('lang_next_label').'</button>
								</div>
							</div>
						</div>';
							...
							<form id="contact-form" method="" action="">
								<div class="row mb-4 m-mb-0">
									...
									<div class="offset-md-3 col-md-6">
											<label class="fw-600"> What is your desired annual billing (&euro;) ? </label> <br>';
					 						if(in_array($projection_guest_permission,array('update','all')))
											{
												$html .= '<input type="text" class="mt-1" name="annual_billing" id="annual_billing" placeholder="" value="'.$projection_row->annual_billing.'" pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency">';
											}
											else
											{
												$html .= '<input class="mt-1" name="annual_billing" id="annual_billing" placeholder="" type="text" readonly value="'.$projection_row->annual_billing.'" onkeypress="return isNumbericKey(event);" pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency">';
											}
									$html .= '</div>
								</div>
								...
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										<button type="button"  onclick="load_guest_projection_wizard(\'1\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>
									<div class="col-md-6 col-6 text-right p-0 form-btns">';
										if(in_array($projection_guest_permission,array('update','all')))
										{
											$html .= '<button type="button" class="save-btn" onclick="load_guest_projection_wizard(\''.$new_step.'\',\''.$projection_id.'\',\'save\',\''.$key_url.'\')"> '.$this->lang->line('lang_save_label').'</button>';
										}
										$html .= '<button type="button" onclick="load_guest_projection_wizard(\''.$new_step.'\',\''.$projection_id.'\',\'next\',\''.$key_url.'\')"> '.$this->lang->line('lang_next_label').'</button>
									</div>
								</div>
							</form>
						</div>
	...
	private function load_projection_step_two_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 3;
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_one_html($projection_id,'next',$key_url);
				}
										...
										<div class="row mb-4 m-mb-0">
											<div class="offset-md-3 col-md-6">
													<label class="fw-600"> Please enter average price of all your products or services.</label>';
													 if(in_array($projection_guest_permission,array('update','all')))
													 {
													 	$html .= '<input type="text" name="average_product_price" id="average_product_price" value="'.$projection_row->average_product_price.'" pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency">';
													 }
													 else
													 {
														 $html .= '<input type="text" name="average_product_price" id="average_product_price" value="'.$projection_row->average_product_price.'" placeholder="" readonly>';
													 }
											 $html .= '</div>
										</div>
										<div class="col-md-12 row m-0 p-0">
											<div class="col-md-6 col-6 text-left p-0">
												 <button type="button"  onclick="load_guest_projection_wizard(\'2\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
											</div>
											<div class="col-md-6 col-6 text-right p-0 form-btns">';
												if(in_array($projection_guest_permission,array('update','all')))
												{
	...
	private function load_projection_step_three_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 4;
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_two_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_two_html($projection_id,'next',$key_url);
					...
					$html .= '<div class="col-lg-12 col-md-12">
						<div class="contact_message form form-bg" id="expense">
							...
							<form id="contact-form" method="" action="">
								...
								<div class="row">
									<div class=" col-md-6">
											...
											<div class="col-md-12 row p-0 mb-3 m-mb-0">';
												...
												{
													$html .= '<div class="col-md-9">
														<input name="operating_expenses" id="operating_expenses" value="'.$projection_row->operating_expenses.'" placeholder="" readonly type="text">
													</div>';
												}
								...
								<div class="row">
									<div class=" col-md-6">
											...
											<div class="col-md-12 row p-0 mb-3 m-mb-0">';
												...
												{
													$html .= '<div class="col-md-9">
													  <input name="advertising_expenses" id="advertising_expenses" value="'.$projection_row->advertising_expenses.'" placeholder="" readonly type="text">
													</div>';
												}
												$html .= '<div class="col-md-3">
								 ...
								 <div class="row mb-4">
									<div class=" col-md-6">
											...
											<div class="col-md-12 row p-0 mb-3 m-mb-0">';
												...
												{
													$html .= '<div class="col-md-9">
													  <input name="average_product_sold_cost" id="average_product_sold_cost" value="'.$projection_row->average_product_sold_cost.'" placeholder="" readonly type="text">
													</div>';
												}
												$html .= '<div class="col-md-3">
								...
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										 <button type="button"  onclick="load_guest_projection_wizard(\'3\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>
									<div class="col-md-6 col-6 text-right p-0 form-btns">';
										  if(in_array($projection_guest_permission,array('update','all')))
										  {
	...
	private function load_projection_step_four_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 5;
		...
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_three_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_three_html($projection_id,'next',$key_url);
					...
					$html .= '<div class="col-lg-12 col-md-12">
						<div class="contact_message form form-bg" id="expense">
							...
							<form id="contact-form" method="" action="">
								<div class="row mb-4">
									<div class="offset-md-3 col-md-6">
										  ...
										  <div class="col-md-12 row p-0 mb-3 m-mb-0">';
											  ...
											  {
												   $html .= '<div class="col-md-9">
														<input name="conversion_rate" id="conversion_rate" value="'.$projection_row->conversion_rate.'"  placeholder="" readonly type="text">
												  </div>';
											  }
											  $html .= '<div class="col-md-3">
								...
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										 <button type="button"  onclick="load_guest_projection_wizard(\'4\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>
									<div class="col-md-6 col-6 text-right p-0 form-btns">';
										  if(in_array($projection_guest_permission,array('update','all')))
										  {
	...
	private function load_projection_step_five_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 6;
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_four_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_four_html($projection_id,'next',$key_url);
					...
					$html .= '<div class="col-lg-12 col-md-12">
						<div class="contact_message form form-bg">
							...
							<form id="contact-form" method="" action="">
								...
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										<button type="button"  onclick="load_guest_projection_wizard(\'5\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>
									<div class="col-md-6 col-6 text-right p-0 form-btns">';
										    if(in_array($projection_guest_permission,array('update','all')))
											{
	...
	private function load_projection_step_six_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 7;
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_five_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_five_html($projection_id,'next',$key_url);
							...
							<form id="contact-form" method="" action="">
								...
								<table id="expenses">
									  ...
									  <tr>
										...
										<td>'.$projection_row->total_annual_expenses.' &euro;</td>
									  </tr>
									  <tr>
										<td>Total annual expenses</td>
								...
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										 <button type="button"  onclick="load_guest_projection_wizard(\'6\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>
									<div class="col-md-6 col-6 text-right p-0 form-btns">';
										  if(in_array($projection_guest_permission,array('update','all')))
										  {
	...
	private function load_projection_step_seven_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 8;
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_six_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_six_html($projection_id,'next',$key_url);
					...
					$html .= '<div class="col-lg-12 col-md-12">
						<div class="contact_message form form-bg">
							...
							<form id="contact-form" method="" action="">
								...
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										  <button type="button"  onclick="load_guest_projection_wizard(\'7\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>
									<div class="col-md-6 col-6 text-right p-0 form-btns">';
									       if(in_array($projection_guest_permission,array('update','all')))
										   {
	...
	private function load_projection_step_eight_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 9;
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_seven_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_seven_html($projection_id,'next',$key_url);
					...
					$html .= '<div class="col-lg-12 col-md-12">
						<div class="contact_message form form-bg">
							...
							<form id="contact-form" method="" action="">
								...
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										 <button type="button"  onclick="load_guest_projection_wizard(\'8\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>
									<div class="col-md-6 col-6 text-right p-0 form-btns">';
									      if(in_array($projection_guest_permission,array('update','all')))
										  {
	...
	private function load_projection_step_nine_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 10;
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_eight_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_eight_html($projection_id,'next',$key_url);
					...
					$html .= '<div class="col-lg-12 col-md-12">
						<div class="contact_message form form-bg" id="expense">
							...
							<form id="contact-form" method="" action="">
									...
									<div class="col-md-6">
										...
										<div class="col-md-12 p-0">
											...
											<div class="col-md-12 row p-0 m-mb-0">';
												...
												{
													$html .= '<div class="col-md-9">
														<input name="scenario_one_expense" id="scenario_one_expense" value="'.$projection_row->scenario_one_total_expenses_ratio.'" type="text" readonly>
													</div>';
												}
												$html .= '<div class="col-md-3">
									...
									<div class="col-md-6">
										...
										<div class="col-md-12 p-0">
											...
											<div class="col-md-12 row p-0 m-mb-0">';
												 ...
												 {
													  $html .= '<div class="col-md-9">
															<input name="scenario_two_conversion" id="scenario_two_conversion" value="'.$projection_row->scenario_two_conversion.'" readonly type="tetx">
														</div>';
												 }
												 $html .= '<div class="col-md-3">
									...
									<div class="col-md-6">
										...
										<div class="col-md-12 p-0 mb-2">
											...
											<div class="col-md-12 row p-0 m-mb-0">';
												...
												{
													$html .= '<div class="col-md-9">
														<input name="additional_product_buyer_ratio" id="additional_product_buyer_ratio" value="'.$projection_row->scenario_three_product_buyer_ratio.'" readonly type="text">
													</div>';
												}
												$html .= '<div class="col-md-3">
										...
										<div class="col-md-12 p-0">
											...
											<div class="col-md-12 row p-0 m-mb-0">';
												...
												{
													$html .= '<div class="col-md-9">
														<input name="additional_product_price" id="additional_product_price" value="'.$projection_row->scenario_three_additional_product_price.'" readonly type="text">
													</div>';
												}
												$html .= '<div class="col-md-3">
									   ...
									   <div class="col-md-12 p-0">
											...
											<div class="col-md-12 row p-0 mb-2">';
												...
												{
													$html .= '<div class="col-md-9">
														<input name="multiple_product_buyer_ratio" id="multiple_product_buyer_ratio" value="'.$projection_row->scenario_multiple_product_buyer_ratio.'" readonly type="text">
													</div>';
												}
												$html .= '<div class="col-md-3">
										...
										<div class="col-md-12 p-0 mb-2" id="multiple-product-buyer-ratio-data" style="display:none">
										 ...
										 <div class="col-md-12 p-0">
											...
											<div class="col-md-12 row p-0 m-mb-0">';
												...
												{
													$html .= '<div class="col-md-9">
														<input name="multiple_product_price" id="multiple_product_price" value="'.$projection_row->scenario_multiple_product_price.'" readonly type="text">
													</div>';
												}
												$html .= '<div class="col-md-3">
								...
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										  <button type="button"  onclick="load_guest_projection_wizard(\'9\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>
									<div class="col-md-6 col-6 text-right p-0 form-btns">';
									   if(in_array($projection_guest_permission,array('update','all')))
									   {
	...
	private function load_projection_step_ten_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 11;
		...
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_nine_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_nine_html($projection_id,'next',$key_url);
					...
					$html .= '<div class="col-lg-12 col-md-12">
						<div class="contact_message form form-bg">
							...
							<form id="contact-form" method="" action="">
								...
								<div class="col-dm-12">
									 <table id="expenses">
										  <tr>
											...
											<td>'.custom_number_format($projection_row->conversion_rate).' %</td>
										  </tr>
									</table>
								</div>
								<div class="row">
									<div class="col-md-6">
										<table id="expenses">
										  ...
										  <tr>
											...
											<td>'.$projection_row->weekly_qualified_web_traffic_volume).'</td>
										  </tr>
										  <tr>
								...
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										 <button type="button"  onclick="load_guest_projection_wizard(\'10\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>
									<div class="col-md-6 col-6 text-right p-0 form-btns">';
					                      if(in_array($projection_guest_permission,array('update','all')))
										  {
	...
	public function load_projection_step_eleven_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		$new_step = 12;
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_ten_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_ten_html($projection_id,'next',$key_url);
							...
							<div class="contact_message form form-bg">
								<div class="row mt-3 mb-3">
									...
									<div class="offset-md-2 col-md-10 text-center last-step-btns">
										...
										if(in_array($projection_guest_permission,array('update','all')))
										{
											$html .= '<button type="button" class="save-btn" onclick="load_guest_projection_wizard(\''.$new_step.'\',\''.$projection_id.'\',\'save\',\''.$key_url.'\')"> <i class="fa fa-save" aria-hidden="true"></i> Save</button>';
										}
										$html .= '<a href="javascript:;" onclick="call_url_key_modal(\''.$social_media_path.'\',\''.base64_encode($projection_id).'\');"><button type="button" class="save-btn"><i class="fa fa-share" aria-hidden="true" ></i> Share</button></a>
										...
										if(in_array($projection_guest_permission,array('update','all')))
										{
											$html .= '<select id="currency" name="currency" class="form-control">
										...
										{
											$html .= '<select id="currency" name="currency" disabled="true" class="form-control">
														<option value="">Select currency</option>
														<option value="USD" '.$currency_usd_selected.'>&dollar; - USD</option>
														<option value="GBP" '.$currency_gbp_selected.'>&pound; - GBP</option>
														<option value="EUR" '.$currency_eur_selected.'>&euro; - EUR</option>
													</select>';
											$html .= '<input type="hidden" id="currency" name="currency" value="'.$projection_row->currency.'">';
										}
									$html .= '</div>
								</div>
								<div class="col-md-12 row m-0 p-0">
									<div class="col-md-6 col-6 text-left p-0">
										<button type="button"  onclick="load_guest_projection_wizard(\'11\',\''.$projection_id.'\',\'previous\',\''.$key_url.'\')"> '.$this->lang->line('lang_previous_label').'</button>
									</div>';
									if(in_array($projection_guest_permission,array('update','all')))
									{
										$html .= '<div class="col-md-6 col-6 text-right p-0">
											<button type="button" onclick="load_guest_projection_wizard(\''.$new_step.'\',\''.$projection_id.'\',\'submit\',\''.$key_url.'\')"> '.$this->lang->line('lang_submit_label').'</button>
										</div>';
									}
								$html .= '</div>
							</div>
	...
	public function load_projection_step_twelve_html($projection_id,$action_type,$key_url)
	{
		$this->load->model("Projection_m","projection");
		$html = '';
		if($projection_id>0)
		{
			...
			if(!empty($projection_row))
			{
				...
				if($action_type=='save')
				{
					$html = $this->load_projection_step_eleven_html($projection_id,'next',$key_url);
				}
				elseif($action_type=='previous')
				{
					$html = $this->load_projection_step_eleven_html($projection_id,'next',$key_url);
